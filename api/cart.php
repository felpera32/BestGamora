...
function getImagemPrincipal($idProduto)
{
    ...
    error_log("=== getImagemPrincipal - Produto ID: $idProduto ===");
    
    try {
        // Primeiro, tenta buscar na tabela imagensproduto
        $sql = "SELECT urlImagem FROM imagensproduto WHERE idProduto = ? AND ordemExibicao = 1 AND status = 'Ativa' LIMIT 1";
        $stmt = $conn->prepare($sql);
        
        if (!$stmt) {
            error_log("Erro na preparação da query imagensproduto: " . $conn->error);
            throw new Exception("Erro na preparação da query: " . $conn->error);
        }
        ...
        if ($result->num_rows > 0) {
            $row = $result->fetch_assoc();
            $imagePath = $row['urlImagem'];
            error_log("Imagem encontrada na tabela imagensproduto: $imagePath");
            
            // Verifica se a imagem existe fisicamente
            if (!empty($imagePath)) {
                if (file_exists($imagePath)) {
                    error_log("Arquivo existe: $imagePath");
                    $stmt->close();
                    return $imagePath;
                } else {
                    error_log("Arquivo NÃO existe: $imagePath");
                    error_log("Caminho completo: " . realpath($imagePath));
                    error_log("Diretório atual: " . getcwd());
                }
            } else {
                error_log("urlImagem está vazia");
            }
        } else {
            error_log("Nenhuma imagem encontrada na tabela imagensproduto");
        }
        ...
        // Se não encontrou na imagensproduto, busca na tabela produtos
        $sql = "SELECT imagemPrincipal FROM produtos WHERE idProduto = ? LIMIT 1";
        $stmt = $conn->prepare($sql);
        
        if (!$stmt) {
            error_log("Erro na preparação da query produtos: " . $conn->error);
            throw new Exception("Erro na preparação da query fallback: " . $conn->error);
        }
        ...
        if ($result->num_rows > 0) {
            $row = $result->fetch_assoc();
            $imagePath = $row['imagemPrincipal'];
            error_log("Imagem encontrada na tabela produtos: $imagePath");
            ...
            if (!empty($imagePath)) {
                ...
                error_log("imagemPrincipal está vazia ou NULL");
            }
        } else {
            error_log("Produto não encontrado na tabela produtos");
        }
    ...
    // Retorna imagem padrão se não encontrou nenhuma
    $placeholder = 'imagens/placeholder.jpg';
    error_log("Retornando placeholder: $placeholder");
    error_log("Placeholder existe: " . (file_exists($placeholder) ? 'SIM' : 'NÃO'));
